...
width (Block w h xs) = w
height (Block w h xs) = h
...
justifyV top n xs | n==height = ys
              | n < height = take n ys
              | (not top) && n > height = replicate (n-height) blanks ++ ys -- bottom aligned
              | top && n > height = ys ++ replicate (n-height) blanks       -- top aligned
...
center (b@(Block w l s)) size = lineUp True [left,b,right]
  where delta = size - w
...
beside:: Bool -> Block -> Block -> Block
beside top (Block w1 h1 xs) (Block w2 h2 ys) = Block (w1+w2) height zs
  where height = max h1 h2
        xs1 = justifyV top height xs
        ys1 = justifyV top height ys
        zs = zipWith (++) xs1 ys1
...
lineUp :: Bool -> [Block] -> Block
lineUp top [] = Block 0 0 []
lineUp top (t:ts) = beside top t (lineUp top ts)
...
sep top n xs = lineUp top (punc xs) where
  punc [] = []
  punc [x] = [x]
